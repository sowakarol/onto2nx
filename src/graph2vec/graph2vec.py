...
from onto2nx.owl_rdf import parse_owl_rdf
...
def dataset_reader(path):
    ...
    :param path: URL to the ontology.
    :return graph: The graph object.
    :return features: Features hash table.
    :return name: Name of the graph.
    """
    name = path.split("/")[-1].strip(".rdf")
    graph = parse_owl_rdf(path)
...
def save_embedding(output_path, model, files, dimensions):
    ...
    out = []
    for f in files:
        identifier = f.split("/")[-1].strip(".rdf")
...
def main(args):
    ...
    print (os.getcwd())
    graphs = [ "https://raw.githubusercontent.com/UCDavisLibrary/wine-ontology/master/ontologies/www.w3.org/TR/owl-guide/wine.rdf"]
